...
from smil_utils import Struct, to_np, to_tensor
...
def create(model_path, model_type='smil',
           **kwargs):
    ...
    if model_type.lower() == 'smil':
        return SMIL(model_path, **kwargs)
    else:
    ...
    def __init__(self, model_path, data_struct=None,
        ...
        ''' SMIL model constructor
            ...
            create_betas: bool, optional
                ...
                (default = True).
            betas: torch.tensor, optional, Bx20
        ...
        if data_struct is None:
            if osp.isdir(model_path):
                model_fn = 'smil_web.pkl'
                smil_path = os.path.join(model_path, model_fn)
            else:
                smil_path = model_path
            assert osp.exists(smil_path), 'Path {} does not exist!'.format(
                smil_path)

            with open(smil_path, 'rb') as smil_file:
                data_struct = Struct(**pickle.load(smil_file,
                                                   encoding='latin1'))
        ...
        if vertex_ids is None:
            # SMIL and SMPL-H share the same topology, so any extra joints can
